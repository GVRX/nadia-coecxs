...
int main(int argc, char * argv[]){
  ...
  int original_ny = 1024;

  int padding = 512;

  int nx = original_nx+2*padding;
  int ny = original_ny+2*padding;


  int status;
  Double_2D data_(original_nx, original_ny);
  Double_2D data(nx, ny);
  status = read_dbin("image_files/FCDI_wf_data.dbin", original_nx, original_ny, data_);
  if(!status){
  ...
  for(int i=0; i<padding; i++){
    for(int j=0; j<padding; j++){
      data.set(i, j, 0.0);
      data.set(i+original_nx, j, 0.0);
      data.set(i, j+original_ny, 0.0);
      data.set(i+original_nx, j+original_ny, 0.0);
    }
  }
